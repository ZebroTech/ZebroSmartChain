...
var (
	// Files that end up in the gozebro*.zip archive.
	gozebroArchiveFiles = []string{
		"COPYING",
		executablePath("gozebro"),
	}

	// Files that end up in the gozebro-alltools*.zip archive.
...
func doArchive(cmdline []string) {
	var (
		...
		upload  = flag.String("upload", "", `Destination to upload the archives (usually "gozebrostore/builds")`)
		ext     string
	)
	flag.CommandLine.Parse(cmdline)
	switch *atype {
	...
	var (
		env      = build.Env()
		basegozebro = archiveBasename(*arch, params.ArchiveVersion(env.Commit))
		gozebro     = "gozebro-" + basegozebro + ext
		alltools = "gozebro-alltools-" + basegozebro + ext
	)
	maybeSkipArchive(env)
	if err := build.WriteArchive(gozebro, gozebroArchiveFiles); err != nil {
		log.Fatal(err)
	}
	if err := build.WriteArchive(alltools, allToolsArchiveFiles); err != nil {
		log.Fatal(err)
	}
	for _, archive := range []string{gozebro, alltools} {
...
func archiveUpload(archive string, blobstore string, signer string, signifyVar string) error {
	...
	if signifyVar != "" {
		...
		untrustedComment := "verify with gozebro-release.pub"
...
func doDocker(cmdline []string) {
	...
	//  - ethereum/client-go:latest                            - Pushes to the master branch, GoZebro only
	//  - ethereum/client-go:stable                            - Version tag publish on GitHub, GoZebro only
	//  - ethereum/client-go:alltools-latest                   - Pushes to the master branch, GoZebro & tools
	//  - ethereum/client-go:alltools-stable                   - Version tag publish on GitHub, GoZebro & tools
	//  - ethereum/client-go:release-<major>.<minor>           - Version tag publish on GitHub, GoZebro only
	//  - ethereum/client-go:alltools-release-<major>.<minor>  - Version tag publish on GitHub, GoZebro & tools
	//  - ethereum/client-go:v<major>.<minor>.<patch>          - Version tag publish on GitHub, GoZebro only
	//  - ethereum/client-go:alltools-v<major>.<minor>.<patch> - Version tag publish on GitHub, GoZebro & tools
	var tags []string
	...
	if *image {
		...
		for _, tag := range tags {
			gozebroImage := fmt.Sprintf("%s:%s-%s", *upload, tag, runtime.GOARCH)
			...
			// master branch.
			for _, img := range []string{gozebroImage, toolImage} {
			...
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:TAG", *upload), gozebroImage)
			build.MustRunCommand("docker", "image", "tag", fmt.Sprintf("%s:alltools-TAG", *upload), toolImage)
			build.MustRunCommand("docker", "push", gozebroImage)
	...
	if len(*manifest) != 0 {
		...
		for i := 0; i < 2; i++ { // 2 attempts, second is race check
			...
			for _, tag := range tags {
				for _, arch := range strings.Split(*manifest, ",") {
					gozebroImage := fmt.Sprintf("%s:%s-%s", *upload, tag, arch)
		...
		// Assemble and push the GoZebro manifest image
		for _, tag := range tags {
			gozebroImage := fmt.Sprintf("%s:%s", *upload, tag)

			var gozebroSubImages []string
			for _, arch := range strings.Split(*manifest, ",") {
				gozebroSubImages = append(gozebroSubImages, gozebroImage+"-"+arch)
			}
			build.MustRunCommand("docker", append([]string{"manifest", "create", gozebroImage}, gozebroSubImages...)...)
			build.MustRunCommand("docker", "manifest", "push", gozebroImage)
		}
...
func doDebianSource(cmdline []string) {
	var (
		...
		sshUser  = flag.String("sftp-user", "", `Username for SFTP upload (usually "gozebro-ci")`)
...
func makeWorkdir(wdflag string) string {
	...
	if wdflag != "" {
		...
		wdflag, err = os.MkdirTemp("", "gozebro-build-")
	}
	if err != nil {
		log.Fatal(err)
	}
...
func doWindowsInstaller(cmdline []string) {
	...
	*workdir = makeWorkdir(*workdir)
	...
	for _, file := range allToolsArchiveFiles {
		...
		if filepath.Base(file) == "gozebro.exe" {
			gozebroTool = file
		} else {
	...
	// first section contains the gozebro binary, second section holds the dev tools.
	templateData := map[string]interface{}{
		"License":  "COPYING",
		"GoZebro":     gozebroTool,
		"DevTools": devTools,
	}
	build.Render("build/nsis.gozebro.nsi", filepath.Join(*workdir, "gozebro.nsi"), 0644, nil)
	...
	installer, err := filepath.Abs("gozebro-" + archiveBasename(*arch, params.ArchiveVersion(env.Commit)) + ".exe")
	if err != nil {
	...
	build.MustRunCommand("makensis.exe",
		...
		"/DARCH="+*arch,
		filepath.Join(*workdir, "gozebro.nsi"),
	)
...
func doPurge(cmdline []string) {
	var (
		store = flag.String("store", "", `Destination from where to purge archives (usually "gozebrostore/builds")`)
