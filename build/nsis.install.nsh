Name "gozebro ${MAJORVERSION}.${MINORVERSION}.${BUILDVERSION}" # VERSION variables set through command line arguments
InstallDir "$InstDir"
...
# Install gozebro binary
Section "GoZebro" GOZEBRO_IDX
  SetOutPath $INSTDIR
  file {{.GoZebro}}

  # Create start menu launcher
  createDirectory "$SMPROGRAMS\${APPNAME}"
  createShortCut "$SMPROGRAMS\${APPNAME}\${APPNAME}.lnk" "$INSTDIR\gozebro.exe"
  createShortCut "$SMPROGRAMS\${APPNAME}\Attach.lnk" "$INSTDIR\gozebro.exe" "attach"
  createShortCut "$SMPROGRAMS\${APPNAME}\Uninstall.lnk" "$INSTDIR\uninstall.exe"

  # Firewall - remove rules (if exists)
  SimpleFC::AdvRemoveRule "GoZebro incoming peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "GoZebro outgoing peers (TCP:30303)"
  SimpleFC::AdvRemoveRule "GoZebro UDP discovery (UDP:30303)"

  # Firewall - add rules
  SimpleFC::AdvAddRule "GoZebro incoming peers (TCP:30303)" ""  6 1 1 2147483647 1 "$INSTDIR\gozebro.exe" "" "" "Ethereum" 30303 "" "" ""
  SimpleFC::AdvAddRule "GeGoZebroth outgoing peers (TCP:30303)" ""  6 2 1 2147483647 1 "$INSTDIR\gozebro.exe" "" "" "Ethereum" "" 30303 "" ""
  SimpleFC::AdvAddRule "GoZebro UDP discovery (UDP:30303)" "" 17 2 1 2147483647 1 "$INSTDIR\gozebro.exe" "" "" "Ethereum" "" 30303 "" ""

  # Set default IPC endpoint (https://github.com/ethereum/EIPs/issues/147)
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "R" "HKLM" "\\.\pipe\gozebro.ipc"
  ${EnvVarUpdate} $0 "ETHEREUM_SOCKET" "A" "HKLM" "\\.\pipe\gozebro.ipc"
...
Function GetInstalledSize
  ...
  ${if} ${SectionIsSelected} ${GOZEBRO_IDX}
    SectionGetSize ${GOZEBRO_IDX} $0
