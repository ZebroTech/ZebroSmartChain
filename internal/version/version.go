...
func versionInfo(info *debug.BuildInfo) string {
	// If the main package is from our repo, prefix version with "gozebro".
	if strings.HasPrefix(info.Path, ourPath) {
		return fmt.Sprintf("gozebro %s", info.Main.Version)
	}
	...
	if mod == nil {
		...
		// version.
		return version + fmt.Sprintf("gozebro %s", params.VersionWithMeta)
	}
