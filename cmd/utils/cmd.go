...
func monitorFreeDiskSpace(sigc chan os.Signal, path string, freeDiskSpaceCritical uint64) {
	for {
		...
		if freeSpace < freeDiskSpaceCritical {
			log.Error("Low disk space. Gracefully shutting down GoZebro to prevent database corruption.", "available", common.StorageSize(freeSpace), "path", path)
			sigc <- syscall.SIGTERM
			break
		} else if freeSpace < 2*freeDiskSpaceCritical {
			log.Warn("Disk space is running low. GoZebro will shutdown if disk space runs below critical level.", "available", common.StorageSize(freeSpace), "critical_level", common.StorageSize(freeDiskSpaceCritical), "path", path)
		}
...
type exportHeader struct {
	Magic    string // Always set to 'gozebrodbdump' for disambiguation
	Version  uint64
	Kind     string
	UnixTime uint64
}

const exportMagic = "gozebrodbdump"
const (
	OpBatchAdd = 0
	OpBatchDel = 1
)
