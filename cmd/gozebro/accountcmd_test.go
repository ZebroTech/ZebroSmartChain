...
func TestAccountListEmpty(t *testing.T) {
	gozebro := runGoZebro(t, "account", "list")
	gozebro.ExpectExit()
}
...
`
	...
	{
		gozebro := runGoZebro(t, "account", "list", "--datadir", datadir)
		gozebro.Expect(want)
		gozebro.ExpectExit()
	}
	{
		gozebro := runGoZebro(t, "--datadir", datadir, "account", "list")
		gozebro.Expect(want)
		gozebro.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	gozebro := runGoZebro(t, "account", "new", "--lightkdf")
	defer gozebro.ExpectExit()
	gozebro.Expect(`
...
func TestAccountHelp(t *testing.T) {
	gozebro := runGoZebro(t, "account", "-h")
	gozebro.WaitExit()
	if have, want := gozebro.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	gozebro = runGoZebro(t, "account", "import", "-h")
	gozebro.WaitExit()
	if have, want := gozebro.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	gozebro := runGoZebro(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer gozebro.ExpectExit()
	gozebro.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	gozebro := runGoZebro(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	gozebro := runGoZebro(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer gozebro.ExpectExit()
	gozebro.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(gozebro.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	gozebro := runMinimalGoZebro(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(gozebro.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	gozebro.SetTemplateFunc("keypath", func(file string) string {
