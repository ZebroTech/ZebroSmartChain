...
func (g *gozebrorpc) killAndWait() {
	g.gozebro.Kill()
	g.gozebro.WaitExit()
}

func (g *gozebrorpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.gozebro.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *gozebrorpc) addPeer(peer *gozebrorpc) {
	g.gozebro.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.gozebro.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.gozebro.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.gozebro.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.gozebro.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *gozebrorpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startGoZebroWithIpc(t *testing.T, name string, args ...string) *gozebrorpc {
	ipcName := fmt.Sprintf("gozebro-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	args = append([]string{"--networkid=42", "--port=0", "--authrpc.port", "0", "--ipcpath", ipcName}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)

	g := &gozebrorpc{
		name: name,
		gozebro: runGoZebro(t, args...),
	}
	ipcpath := ipcEndpoint(ipcName, g.gozebro.Datadir)
	// We can't know exactly how long gozebro will take to start, so we try 10
...
func initGoZebro(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing gozebro: %v ", args)
	g := runGoZebro(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
...
func startLightServer(t *testing.T) *gozebrorpc {
	datadir := initGoZebro(t)
	t.Logf("Importing keys to gozebro")
	runGoZebro(t, "account", "import", "--datadir", datadir, "--password", "./testdata/password.txt", "--lightkdf", "./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startGoZebroWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--miner.etherbase=0x02f0d131f1f97aef08aec6e3291b957d9efe7105", "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *gozebrorpc {
	datadir := initGoZebro(t)
	return startGoZebroWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
