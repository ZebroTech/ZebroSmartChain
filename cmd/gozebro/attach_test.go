...
// TestAttachWithHeaders tests that 'gozebro attach' with custom headers works, i.e
...
// TestAttachWithHeaders tests that 'gozebro db --remotedb' with custom headers works, i.e
// that custom headers are forwarded to the target.
...
func testReceiveHeaders(t *testing.T, ln net.Listener, gozebroArgs ...string) {
	var ok uint32
	server := &http.Server{
		Addr: "localhost:0",
	...
	go server.Serve(ln)
	defer server.Close()
	runGoZebro(t, gozebroArgs...).WaitExit()
