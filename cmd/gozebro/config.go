...
func loadConfig(file string, cfg *gozebroConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.IPCPath = "gozebro.ipc"
	return cfg
}

// makeConfigNode loads gozebro configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, gozebroConfig) {
	// Load defaults.
	cfg := gozebroConfig{
...
// makeFullNode loads gozebro configuration and creates the Ethereum backend.
...
func applyMetricConfig(ctx *cli.Context, cfg *gozebroConfig) {
