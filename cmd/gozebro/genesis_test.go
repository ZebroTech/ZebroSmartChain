...
// Tests that initializing GoZebro with a custom genesis block and chain definitions
// work properly.
func TestCustomGenesis(t *testing.T) {
	...
	for i, tt := range customGenesisTests {
		...
		runGoZebro(t, "--datadir", datadir, "init", json).WaitExit()

		// Query the custom genesis block
		gozebro := runGoZebro(t, "--networkid", "1337", "--syncmode=full", "--cache", "16",
			"--datadir", datadir, "--maxpeers", "0", "--port", "0", "--authrpc.port", "0",
			"--nodiscover", "--nat", "none", "--ipcdisable",
			"--exec", tt.query, "console")
		gozebro.ExpectRegexp(tt.result)
		gozebro.ExpectExit()
	}
}
...
func TestCustomBackend(t *testing.T) {
	...
	testfunc := func(t *testing.T, tt backendTest) error {
		...
		{ // Init
			...
			gozebro.ExpectRegexp(tt.initExpect)
			gozebro.ExpectExit()
		}
		{ // Exec + query
			...
			gozebro := runGoZebro(t, args...)
			gozebro.ExpectRegexp(tt.execExpect)
			gozebro.ExpectExit()
		}
		return nil
	}
