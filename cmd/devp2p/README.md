...
1. initialize the gozebro node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run gozebro with the following flags:
```
gozebro --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the gozebro node:
 ```
...
To run the eth66 protocol test suite, initialize a gozebro node as described above and run the following command,
replacing `<enode>` with the enode of the gozebro node:

 ```
 devp2p rlpx eth66-test <enode> cmd/devp2p/internal/ethtest/testdata/chain.rlp cmd/devp2p/internal/ethtest/testdata/genesis.json
```

[eth]: https://github.com/ZebroTech/devp2p/blob/master/caps/eth.md
[dns-tutorial]: https://gozebro.zebrocoin.info/docs/developers/gozebro-developer/dns-discovery-setup
[discv4]: https://github.com/Zebrotech/devp2p/tree/master/discv4.md
