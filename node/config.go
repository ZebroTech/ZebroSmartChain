...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of gozebro is "gozebro". If no
	...
	oldGoZebroResourceWarning bool
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "GoZebro", keep that.
	if name == "gozebro" || name == "gozebro-testnet" {
		name = "GoZebro"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "gozebro" instances.
var isOldGoZebroResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) ResolvePath(path string) string {
	...
	// by gozebro 1.4 are used if they exist.
	if warn, isOld := isOldGoZebroResource[path]; isOld {
		oldpath := ""
		if c.name() == "gozebro" {
			oldpath = filepath.Join(c.DataDir, path)
		}
		if oldpath != "" && common.FileExist(oldpath) {
			if warn && !c.oldGoZebroResourceWarning {
				c.oldGoZebroResourceWarning = true
				log.Warn("Using deprecated resource file, please move this file to the 'gozebro' subdirectory of datadir.", "file", oldpath)
			}
			return oldpath
		}
	}
