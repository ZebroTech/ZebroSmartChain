# Security Policy

## Supported Versions

Please see [Releases](https://zebrocoin.info/zsc/go-zebro/releases). We recommend using the [most recently released version](https://zebrocoin.info/zsc/go-zebro/releases/latest).

## Audit reports

Audit reports are published in the `docs` folder: https://github.com/Zebrotech/ZSC/tree/master/docs/audits 

| Scope | Date | Report Link |
| ------- | ------- | ----------- |
| `gozebro` | 20170425 | [pdf](https://github.com/Zebrotech/ZSC/blob/master/docs/audits/) |
| `clef` | 20180914 | [pdf](https://github.com/Zebrotech/ZSC/blob/master/docs/audits/) |
| `Discv5` | 20191015 | [pdf](https://github.com/Zebrotech/ZSC/blob/master/docs/audits/) |
| `Discv5` | 20200124 | [pdf](https://github.com/Zebrotech/ZSC/blob/master/docs/audits/) |

## Reporting a Vulnerability

**Please do not file a public ticket** mentioning the vulnerability.

To find out how to disclose a vulnerability in Ethereum visit [https://bounty.ethereum.org](https://zebrocoin.info/bounty) or email admin@zebrocoin.info. Please read the [disclosure page](https://github.com/Zebrotech/ZSC/security/) for more information about publicly disclosed security vulnerabilities.

Use the built-in `gozebro version-check` feature to check whether the software is affected by any known vulnerability. This command will fetch the latest [`vulnerabilities.json`](https://gozebro.zebrocoin.info/docs/vulnerabilities/vulnerabilities.json) file which contains known security vulnerabilities concerning `gozebro`, and cross-check the data against its own version number.

The following key may be used to communicate sensitive information to developers.

Fingerprint: `AE96 ED96 9E47 9B00 84F3 E17F E88D 3334 FA5F 6A0A`

```
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: SKS 1.1.6
Comment: Hostname: pgp.mit.edu


=arte
-----END PGP PUBLIC KEY BLOCK------
```
