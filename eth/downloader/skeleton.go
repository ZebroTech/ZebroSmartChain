...
// the process. This is usually the case when GoZebro is shutting down and some events
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but GoZebro is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until GoZebro requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
